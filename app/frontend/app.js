...
VIEWS.accounts = async () => {
  ...
  <div class="card" style="margin-top:14px"><h3>${t('acc_would_see')}</h3>
    ...
    <div style="margin-top:10px;padding:10px 12px;border-radius:8px;background:var(--ink);font-size:12px;border:1px solid var(--line-strong);color:var(--dim);line-height:1.5">
      ${deviceOnline ? '✅ ' + t('acc_sentinel_on') : '⚠️ ' + t('acc_sentinel_off') + ` <a onclick="location.hash='#devices'" style="color:var(--gold);cursor:pointer">${t('dev_get_app')}</a>`}
    </div></div>`);
